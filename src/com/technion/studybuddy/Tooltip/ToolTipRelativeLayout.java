...
import com.technion.studybuddy.Tooltip.exception.NoOverflowMenuRuntimeException;
import com.technion.studybuddy.Tooltip.exception.NoTitleViewRuntimeException;
import com.technion.studybuddy.Tooltip.exception.ViewNotFoundRuntimeException;
...
public class ToolTipRelativeLayout extends RelativeLayout
{
			...
			final ToolTip toolTip)
	{
		final int titleResId = Resources.getSystem().getIdentifier(
				ToolTipRelativeLayout.ACTION_BAR_TITLE,
				ToolTipRelativeLayout.ID, ToolTipRelativeLayout.ANDROID);
		if (titleResId == 0)
	...
	private static View findActionBarOverflowMenuView(final Activity activity)
	{
		...
		final int actionBarViewResId = Resources.getSystem().getIdentifier(
				ToolTipRelativeLayout.ACTION_BAR, ToolTipRelativeLayout.ID,
				ToolTipRelativeLayout.ANDROID);
		...
		for (int i = 0; i < actionBarViewChildCount; ++i)
			if (actionBarView.getChildAt(i).getClass().getSimpleName()
					.equals(ToolTipRelativeLayout.ACTION_MENU_VIEW))
		...
		for (int i = 0; i < actionMenuChildCount; ++i)
			if (actionMenuView.getChildAt(i).getClass().getSimpleName()
					.equals(ToolTipRelativeLayout.OVERFLOW_MENU_BUTTON))
